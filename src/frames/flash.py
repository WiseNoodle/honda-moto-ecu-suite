...
class HondaECUFlashPanel(HondaECUAppPanel):
    ...
    def Build(self):
        ...
        self.optsbox.Add(self.wchecksuml, 0, flag=wx.ALIGN_CENTER_VERTICAL | wx.LEFT, border=10)
    ...
    def OnGo(self, _event):
        ...
        if self.modebox.GetSelection() == 0:
            ...
            self.progressboxp.Show()
            self.Layout()
            print("[FLASH] Sending ReadPanel:", repr(data), "offset:", hex(offset))
    ...
    def OnValidateMode(self, event):
        ...
        if "state" in self.parent.ecuinfo:
            if self.modebox.GetSelection() == 0:
                if self.parent.ecuinfo["state"] in [ECUSTATE.OK, ECUSTATE.SECURE]:
                    offset = None
                    try:
                        offset = int(self.offset.GetValue(), 16)
                    except TypeError:
